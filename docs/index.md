---
layout: home

hero:
  name: 周周黑客松
  text: 活动执行 SOP
  tagline: 简单、高效、有温度的活动执行指南。助力每一位创造者。
  actions:
    - theme: brand
      text: 🎛️ 活动统筹清单 (Organizer)
      link: "/roles/organizer"
    - theme: alt
      text: 🎯 获取模板 (Template)
      link: "/manual/activity-template"

features:
  - title: 🚀 极速上手
    details: 模块化的 SOP 设计，让第一次参与的志愿者也能在 5 分钟内掌握核心要点。
  - title: 🤖 AI 赋能
    details: 内置多种 AI 提示词，协助主理人写文案、写脚本、做复盘，解放生产力。
  - title: 🤝 温暖连接
    details: 不只是冷冰冰的流程，更强调人与人之间的连接与社区文化的传递。
---

## 快速导航 {#navigation}

> **💡 温馨提示**：请找到你本次活动被分配的角色，点击角色名称进行深度阅读。建议在活动开始前 24 小时完成阅读。

### 🏃 核心保障组
| 角色                                | 核心职责                           |
| :---------------------------------- | :--------------------------------- |
| [**主持人**](./roles/mc)            | 氛围带动、环节串联、连接参与者     |
| [**物料/场地**](./roles/logistics/) | 物料准备、场地布置、清场复原       |
| [**签到接待**](./roles/logistics/check-in) | 入场核销、人流引导、第一印象 |
| [**宣发推送**](./roles/pr)          | 现场素材捕捉、群消息同步、成果传播 |

### 🛠️ 技术与内容组 (按需)
| 角色                                 | 核心职责                     |
| :----------------------------------- | :--------------------------- |
| [**技术导师 Mentor**](./roles/tech-support) | 教程演示、巡场辅导、节奏推进 |
| [**现场中控**](./roles/operator)     | 投屏、音乐、灯光、路演管理、计时 |
| [**分享嘉宾（可选）**](./roles/speaker) | 主题分享、控时、互动答疑  |

### 🤝 统筹与外联 (管理)
| 角色                                         | 核心职责                     |
| :------------------------------------------- | :--------------------------- |
| [**活动主理人/统筹**](/roles/organizer) | 全局掌控、资源协调、突发决策 |
| [**赞助商对接**](./roles/sponsor)            | 赞助权益落地、大客户接待     |


## 常用资源 {#resources}

> **🔗 常用链接**：方便志愿者与组织者快速获取社区资源。

| 资源名称     | 访问地址                                                                                        |
| :----------- | :---------------------------------------------------------------------------------------------- |
| **加入群聊** | [join.hackathonweekly.com](https://join.hackathonweekly.com)                                    |
| **场地预约** | [space.hackathonweekly.com](https://space.hackathonweekly.com)                                  |
| **物料清单** | [📦 活动物料清单](/manual/materials)                                                             |
| **设计素材** | [Logo/排版素材汇总](http://logo.hackathonweekly.com)                                            |
| **费用报销** | [社区报销申请表](https://hackathonweekly.feishu.cn/share/base/form/shrcnSg2UVWbBqh6qV4xwSHPi1c) |
| **反馈建议** | [feedback.hackathonweekly.com](http://feedback.hackathonweekly.com)                             |


## 志愿者须知 {#volunteer-notices}

作为周周黑客松的志愿者，我们需要共同遵守以下准则：

1.  **准时到达**：请务必按照规定的集合时间到达场地。
2.  **主动协助**：在完成自己岗位工作之余，主动观察现场是否有需要帮忙的地方。
3.  **保持热情**：我们是一个温暖的社区，请微笑面对每一位参与者。
4.  **熟悉 SOP**：在活动开始前，请至少完整阅读一遍你的角色 SOP。
5.  **安全第一**：遇到任何突发状况或安全问题，第一时间反馈给活动主理人。


**版本信息**：v3.1 (2026-01-03 优化版)
**更新日志**：
- 重新设计首页结构，强化志愿者导航功能
- 增加社区介绍与志愿者须知
- 区分组织者与志愿者入口


本文档网站基于 https://github.com/01MVP/01mvp-docs-starter 模板构建！
